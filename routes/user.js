        ...
        if (userId) {
            if (user.love != {}) {
                if (user.love.type === "Married") {
                    ...
                    user.isMarried = (await models.LoveRequest.findOne({ userId: reqUserId, targetId: userId, type: "Married" })) != null;
                    if (!user.isMarried) {
                        user.isLove = true;
                    }
                }
                ...
                user.isLove = (await models.LoveRequest.findOne({ userId: reqUserId, targetId: userId, type: "Lover" })) != null;
            }
        }
...
async function acceptLove(userId, userIdToLove, type, userName){
    var userToLove = await models.User.findOne({ id: userIdToLove })
    .select("lastActive");
    ...
    if(type === "Married"){
        await models.Love.updateOne({ userId: userIdToLove}, {$set: {type: "Married"}}).exec();
        await models.Love.updateOne({ userId: userId}, {$set: {type: "Married"}}).exec();
    ...
    await models.User.updateOne({ id: userId }, { $set: { love: userIdToLove }}).exec();
    await models.User.updateOne({ id: userIdToLove }, { $set: { love: userId } }).exec();

}


//STILL NEED TO WRITE CASE FOR BREAKING UP WHEN OTHER USER HAS PROPOSED.
//I.E. IF CURRENT LOVERS, PARTNER PROPOSED MARRIAGE, BUT CURRENT USER BREAKS UP.
router.post("/love", async function (req, res) {
    ...
    try {
        ...
        if(userId === userIdToLove){
            res.status(500);
            res.send({message: "Self love is good, but the site doesn't work that way.", love: null});
            return;
        }
        var existingRequest = await models.LoveRequest.findOne({ userId, targetId: userIdToLove })
            .select("type -_id");
        ...
        if(existingRequest) {
            ...
            await models.LoveRequest.deleteOne({ userId, targetId: userIdToLove}).exec();
            res.send({message: cancelMessage, love: null});
            return;
        }

        var existingLove = await models.Love.findOne({ userId, loveId: userIdToLove})
            .select("type _id");

        if(currentLove === "Married" && existingLove.toJSON().type === "Married" && requestType === "Marry"){
            await unlove(userId, userIdToLove, "Married");
            res.send({message: "Divorced this user.", love: null});
            return;
        }
        else{
            // Unlove
            if(existingLove !== null && existingLove.toJSON.type === "Lover" &&  currentLove === "Lover" && requestType === "Love"){
                await unlove(userId, userIdToLove, "Lover");
                res.send({message: "Broke up with this user.", love: null});
                return;
                // await models.Love.deleteOne({ userId, loveId: userIdToLove }).exec();
                // await models.Love.deleteOne({ userId: userIdToLove, loveId: userId }).exec();
                ...
                // res.send("Broke up with this user.");
            }
        }

        existingRequest = await models.LoveRequest.findOne({ userId: userIdToLove, targetId: userId})
            .select("type _id");
        ...
        if(existingRequest){
            ...
            acceptLove(userId, userIdToLove, requestType, userName);
            var love = await models.Love.findOne({ userId })
                        .select("loveId type")
                        .populate({
                        path: "love",
            ...
            if (requestType === "Lover") {
                res.send({message: "Love request accepted!", love: love.love});
            }
            else if (requestType === "Married") {
                res.send({message: "Marriage proposal accepted!", love: love.love});
            }
            ...
            // await models.LoveRequest.deleteOne({ userId: userIdToLove, targetId: userId }).exec();
            ...
            //     link: `/user/${userId}`
            // }, [userIdToLove]);
            // res.send("Love request accepted!");
